...
FluidCapsule 1.0 intentionally sets `minSdk = 36`. Older Android versions and other OEM devices are outside the supported scope.
...
FluidCapsule 1.0 有意将 `minSdk` 设为 36；旧版 Android 和其他厂商设备不在支持范围内。
...
Run the five CPH2797 instrumentation checks / 运行 5 项 CPH2797 仪器测试：
...
- Version 1.0 supports only CPH2797 on the verified Android 16 firmware baseline.
...
- 1.0 仅支持经过验证的 Android 16 固件基线上的 CPH2797。
...
- Local JUnit suite: 42 tests.
- CPH2797 instrumentation suite: 5 tests, including continuous history scrolling and system notification queue preemption/restoration.
...
- 本地 JUnit 测试：42 项。
- CPH2797 仪器测试：5 项，包括历史页连续滚动容器以及系统通知队列的抢占/恢复。
